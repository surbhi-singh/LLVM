...
void CWriter::printCast(unsigned opc, const Type *SrcTy, const Type *DstTy) {
  ...
  switch (opc) {
    ...
    case Instruction::FPToUI: // For these, make sure we get an unsigned dest
      Out << '(';
      if(isIf) printSimpleType(Out, DstTy, false);
      Out << ')';
      break;
    case Instruction::SExt:
    case Instruction::FPToSI: // For these, make sure we get a signed dest
      Out << '(';
      if(isIf) printSimpleType(Out, DstTy, true);
      Out << ')';
      break;
    default:
  ...
  switch (opc) {
    ...
    case Instruction::ZExt:
      Out << '(';
      if(isIf) printSimpleType(Out, SrcTy, false);
      Out << ')';
      break;
    case Instruction::SIToFP:
    case Instruction::SExt:
      Out << '(';
      if(isIf) printSimpleType(Out, SrcTy, true);
      Out << ')';
      break;
    case Instruction::IntToPtr:
    case Instruction::PtrToInt:
      // Avoid "cast to pointer from integer of different size" warnings
      if(isIf) Out << "(unsigned long)";
      break;
    case Instruction::Trunc:
    case Instruction::BitCast:
    case Instruction::FPExt:
...
void CWriter::printConstant(Constant *CPV, bool Static) {
  if (const ConstantExpr *CE = dyn_cast<ConstantExpr>(CPV)) {
    ...
    case Instruction::BitCast:
      Out << "(";
      if(isIf) printCast(CE->getOpcode(), CE->getOperand(0)->getType(), CE->getType());
...
std::string CWriter::GetValueName(const Value *Operand) {
 ...
 if(isIf)  return "llvm_cbe_" + VarName;
 else {
 	isIf = true;
 	return ")";
 	}
...
bool CWriter::doInitialization(Module &M) {
  ...
  if (!M.global_empty()) {
    ...
    for (Module::global_iterator I = M.global_begin(), E = M.global_end();
      ...
      if(isIf) printType(Out, I->getType()->getElementType(), false, GetValueName(I));
  ...
  if (!M.global_empty()) {
    ...
    for (Module::global_iterator I = M.global_begin(), E = M.global_end();
      ...
      if (!I->isDeclaration()) {
        ...
        if (isIf) printType(Out, I->getType()->getElementType(), false,
                  GetValueName(I));
  ...
  // predicates
  Out << "proc static inline int llvm_fcmp_ord(double X, double Y) { ";
  Out << "return X = X && Y = Y; }\n";
  
  Out << "proc static inline int llvm_fcmp_uno(double X, double Y) { ";
  Out << "return X != X || Y != Y; }\n";
  
  Out << "proc static inline int llvm_fcmp_ueq(double X, double Y) { ";
  Out << "return X = Y || llvm_fcmp_uno(X, Y); }\n";
  
  Out << "proc static inline int llvm_fcmp_une(double X, double Y) { ";
  Out << "return X != Y; }\n";
  
  Out << "proc static inline int llvm_fcmp_ult(double X, double Y) { ";
  Out << "return X <.  Y || llvm_fcmp_uno(X, Y); }\n";
  
  Out << "proc static inline int llvm_fcmp_ugt(double X, double Y) { ";
  Out << "return X >.  Y || llvm_fcmp_uno(X, Y); }\n";
  
  Out << "proc static inline int llvm_fcmp_ule(double X, double Y) { ";
  Out << "return X <=. Y || llvm_fcmp_uno(X, Y); }\n";
  
  Out << "proc static inline int llvm_fcmp_uge(double X, double Y) { ";
  Out << "return X >=. Y || llvm_fcmp_uno(X, Y); }\n";
  
  Out << "proc static inline int llvm_fcmp_oeq(double X, double Y) { ";
  Out << "return X = Y ; }\n";
  
  Out << "proc static inline int llvm_fcmp_one(double X, double Y) { ";
  Out << "return X != Y && llvm_fcmp_ord(X, Y); }\n";
  
  Out << "proc static inline int llvm_fcmp_olt(double X, double Y) { ";
  Out << "return X <.  Y ; }\n";
  
  Out << "proc static inline int llvm_fcmp_ogt(double X, double Y) { ";
  Out << "return X >.  Y ; }\n";
  
  Out << "proc static inline int llvm_fcmp_ole(double X, double Y) { ";
  Out << "return X <=. Y ; }\n";
  
  Out << "proc static inline int llvm_fcmp_oge(double X, double Y) { ";
  Out << "return X >=. Y ; }\n";
  return false;
}
...
bool isNotDigit( unsigned char ch)
{
   return (isdigit(ch) == 0);
 }
void CWriter::printFunction(Function &F) {
std::string const name1( "llvm_cbe_pre" );
std::string const name2( "llvm_cbe_post" );
 ...
 for( Function::iterator BB = F.begin(), E = F.end(); BB != E ; BB++)
    {
    	std::string name3 = GetValueName(BB);
	
  if(	(name3.size() >= name2.size()
    && std::equal( name2.begin(), name2.end() - 1, name3.begin() )
    && ((std::find_if( name3.begin() + name2.size(),
                     name3.end(), isNotDigit )) == name3.end()) )
                    
                     
   ||    (name3.size() >= name1.size()
    && std::equal( name1.begin(), name1.end() - 1, name3.begin() )
    && ((std::find_if( name3.begin() + name1.size(),
                     name3.end(), isNotDigit )) == name3.end()))
     )      
        {
  ...
  std::string const name1( "llvm_cbe_pre" );
std::string const name2( "llvm_cbe_post" );
std::string const name4( "llvm_cbe_False" );

    if(name3.size() >= name4.size()
    	&& std::equal( name4.begin(), name4.end() - 1, name3.begin() )
    	&& ((std::find_if( name3.begin() + name4.size(),
                     name3.end(), isNotDigit )) == name3.end())
        ) continue;
...
else{if (NeedsLabel) Out << GetValueName(BB) << " :\n";
...
/*if( (name3.size() >= name1.size()
    && std::equal( name1.begin(), name1.end(), name3.begin() )
    && ((std::find_if( name3.begin() + name1.size(),
                     name3.end(), isNotDigit )) == name3.end()) ) 
                    ...
                    ||
                     ...
                     (name3.size() >= name2.size()
    && std::equal( name2.begin(), name2.end(), name3.begin() )
    && ((std::find_if( name3.begin() + name2.size(),
                     name3.end(), isNotDigit )) == name3.end()) ) )
...
void CWriter::visitBranchInst(BranchInst &I) {
  ...
  if (I.isConditional()) {
    if (isGotoCodeNecessary(I.getParent(), I.getSuccessor(0))) {
      if(IsIf && isneed ) Out << "  if (";
      if(isneed)  writeOperand(I.getCondition());
       if(IsIf && isneed ) Out << ") {\n";

      if(IsIf) printPHICopiesForSuccessor (I.getParent(), I.getSuccessor(0), 2);
      if(IsIf) printBranchToBlock(I.getParent(), I.getSuccessor(0), 2);

      if (isGotoCodeNecessary(I.getParent(), I.getSuccessor(1))) {
         if(IsIf && isneed){ Out << "  } else {\n";
...
void CWriter::visitBinaryOperator(Instruction &I) {
  ...
  if ((I.getType() == Type::getInt8Ty(I.getContext())) ||
    ...
    needsCast = true;
    Out << "((";
    if(isIf) printType(Out, I.getType(), false);
    Out << ")(";
  }
...
#undef GET_GCC_BUILTIN_NAME
  ...
  case Intrinsic::x86_sse2_cmp_pd:
    Out << '(';
    if(isIf) printType(Out, I.getType());
    Out << ')';
...
void CWriter::writeMemoryAccess(Value *Operand, const Type *OperandType,
  ...
  if (IsVolatile || IsUnaligned) {
    ...
    if(isIf) printType(Out, OperandType, false, IsUnaligned ? "data" : "volatile*");
    if (IsUnaligned) {
      Out << "; } ";
